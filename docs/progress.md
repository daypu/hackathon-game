...
## 会话 12 - 2026-06-07（陷地终极修复 + 角色切换 bug + 色度抠图重处理）

### 背景
会话11结束后，陷地问题虽找到 y 坐标根因，但效果仍不对。kafka 提供新的 wukong.png/bajie.png。

### 主要工作

#### 1. 色度抠图重处理（2 轮）
- 第1轮：色度抠图 wukong（108万白像素透明化）、bajie（111万白像素透明化）
- 第2轮：kafka 换新图后再次处理 wukong（49万）、bajie（63万）
- 结果：白色背景被去除，但脚仍然不能贴地

#### 2. 坐标调试（数据驱动）
kafka 提示后加 console.log 输出关键坐标：

| 对象 | visual.y | body.y |
|---|---|---|
| 地面基线 | groundY=460 | — |
| 地面物理体 | groundBody.y=500 | body.y=460 |
| 悟空 | 460 | 400 |
| 八戒 | 460 | 392 |
| 陆地妖怪 | 460 | — |

**关键发现**：角色 visual.y 都正确（460=地面顶），但 body.y 偏上 60-68px。物理体不在正确位置。

#### 3. 角色切换 bug 发现
kafka 反馈：从八戒切回悟空会掉到地面下

调试日志显示坐标累积偏移：460→466→473

#### 4. 3 轮误修复
1. **body.setSize + setOffset**（悟空 offsetY=12、八戒=14）→ 物理体更靠下
2. **调小 offsetY**（设 0 和 -20）→ kafka 提示方向对了但不够
3. **地面物理体顶部锚点**（setOrigin(0.5,0) + y=groundY）→ 角色在地面上方飘着

#### 5. kafka 拍板修复（关键转折）
**kafka 一句话**："物理地面坐标改了重力会自动落到正确位置吧？"

最终方案：**地面物理体设 y = groundY - 50** + setOrigin(0.5,0)
- 角色受重力落下，脚底正好贴视觉地面顶
- **不再调角色 body 的任意参数**

#### 6. 切换 bug 修复
根因：直接复制 visual.y 导致累积偏移（不同 size 的角色位置错乱）
修复：重置 y = groundY，只继承 x 速度，y 速度清零

#### 7. 妖怪尺寸放大
kafka 要求：飞行妖怪和陆地妖怪从 size=40 放大到 size=55（跑酷+Boss战两阶段同步）

### 修改文件清单
| 文件 | 修改 |
|---|---|
| `assets/images/wukong.png` | 色度抠图（旧备份删掉） |
| `assets/images/bajie.png` | 色度抠图（旧备份删掉） |
| `GameScene.js` | 地面物理体重写 + 切换逻辑重写 + 坐标调试输出 |
| `Wukong.js` | body.setSize + setOffset 设回合理值 |
| `Bajie.js` | body.setSize + setOffset 设回合理值 |
| `Obstacle.js` | 妖怪 size 40→55 |
| `BossMonster.js` | 妖怪 size 40→55 |

### 关键经验
1. **调试说服力远大于猜测**：加 console.log 打印坐标让问题从"猜"变"看"
2. **kafka 的"物理地面坐标改了让重力自动处理"是一句解千愁**：7+ 次调 body/setOrigin/setOffset 都不如改物理地面顶部位置
3. **角色切换时不能复制所有属性**：不同 size 角色的坐标语义不同，重置到已知位置更可靠
4. **对比坐标法**：正常（陆地妖怪 y=460）vs 异常（悟空 y=460 但 body.y=400）→ 差异在 body 位置错位
5. **本项目提交完整性好**：kafka 已把修复提交为 6.0（最新 HEAD）
