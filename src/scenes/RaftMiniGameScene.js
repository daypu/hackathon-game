...
export class RaftMiniGameScene {
  ...
  update(dt) {
    ...
    const waveTorque = s.waveWarning && s.waveWarning.timer <= 0 ? 0 : 0;

    s.raftVelocity += axis * control * dt;
    s.raftVelocity += (drift + fearShake + monsterTorque + waveTorque) * dt;
    ...
    let speedMul = 1;
    if (absA < 15) speedMul = 1.08;
    else if (absA > 30) speedMul = 0.88;
    s.progress = clamp(s.progress + (100 / ROUND_SECONDS) * speedMul * dt, 0, 100);
  ...
  _writeResult(success, reason) {
    const s = this.state;
    if (this.g.shared.raftResult && this.g.shared.raftResult.reason === reason && this.g.shared.raftResult.success === success) {
      return;
    }
    ...
    if (success) this.g.shared.raftCleared = true;
  }

  _returnToPlay() {
    const back = this.opts.returnScene;
    if (!back) return;
    back._resume = true;
    const res = this.g.shared.raftResult;
    if (res?.success) {
      back.message = `木筏渡河完成：${res.rating}`;
      back.messageTimer = 3.2;
    } else if (res) {
      back.message = '返回流沙河，可再次尝试木筏渡河。';
      back.messageTimer = 3.2;
    }
  ...
  _tryComfort() {
    ...
    s.comfortCooldown = 5;
    s.comfortCount += 1;
    this._say('悟空：师父莫慌，有俺老孙在！', 3.2);
  }

  _tryStartRepair() {
    const s = this.state;
    if (s.repairCooldown > 0) {
      this._say(`修理冷却中：${s.repairCooldown.toFixed(1)} 秒`);
      return;
    }
    if (s.raftHP >= 100) {
      this._say('木筏暂时不需要修理。');
    ...
    if (Math.abs(s.raftAngle) > 15) {
      this._say('木筏太晃！先稳住再按 1！');
      return;
    }
    s.repairing = true;
    s.repairTimer = 1.2;
    s.repairCooldown = 4;
    this._say('修理中，保持平衡！', 2.4);
  }
  ...
  _updateMonster(dt) {
    ...
    if (!s.monsterActive) {
      ...
      if (s.progress > 35 && s.monsterSpawnTimer <= 0) {
        ...
        this._say(`${this._sideLabel(s.monsterSide)}河怪！按 2！`, 2.2);
      }
      return;
    }
  ...
  _updateWaves(dt) {
    ...
    if (!s.waveWarning) {
      ...
      if (s.progress > 15 && s.waveSpawnTimer <= 0) {
        ...
        this._say(`${this._sideLabel(s.waveWarning.side)}水浪预警！稳住！`, 1.8);
      }
      return;
    }
  ...
  _drawTopHud(r, s) {
    ...
    r.roundRect(hx, by + 20, 170, 12, 6, 'rgba(10,8,18,0.85)', 'rgba(0,0,0,0.6)', 2);
    const hpRatio = clamp(s.raftHP / 100, 0, 1);
    const hpColor = s.raftHP > 60 ? '#7ed957' : s.raftHP > 30 ? '#ffce54' : '#ff7a6a';
    r.roundRect(hx + 2, by + 22, (170 - 4) * hpRatio, 8, 4, hpColor);

    const fx = 720;
    r.text(`惊慌 ${Math.round(s.monkFear)}`, fx, by + 14, { size: 13, color: '#ff9a9a', weight: '800' });
    r.roundRect(fx, by + 20, 210, 12, 6, 'rgba(10,8,18,0.85)', 'rgba(0,0,0,0.6)', 2);
    const fearRatio = clamp(s.monkFear / 100, 0, 1);
    const fearColor = s.monkFear < 40 ? '#7ed957' : s.monkFear < 70 ? '#ffce54' : '#ff7a6a';
    r.roundRect(fx + 2, by + 22, (210 - 4) * fearRatio, 8, 4, fearColor);

    const cx = GAME.width - 210;
    const cy = 52;
    r.text('平衡', cx - 12, cy + 14, { size: 13, color: '#9fd0ff', align: 'right', weight: '800' });
    r.roundRect(cx, cy + 4, 180, 12, 6, 'rgba(10,8,18,0.85)', 'rgba(0,0,0,0.6)', 2);
    const ratio = clamp((s.raftAngle + 55) / 110, 0, 1);
    r.roundRect(cx + 2, cy + 6, (180 - 4) * ratio, 8, 4, danger ? '#ff7a6a' : '#4aa3ff');
  }
  ...
  _drawRaft(r, s) {
    ...
    if (s.monkFear >= 70) {
      ...
      r.text('唐僧慌了！按 Space', GAME.width / 2 + 66, 232, {
        size: 12,
        color: '#fff2b0',
        align: 'center',
        weight: '900',
  ...
  _drawBottomHud(r, s) {
    ...
    const attackSub = s.attackCooldown > 0
      ? `冷却 ${s.attackCooldown.toFixed(1)}s`
      : s.monsterActive
        ? '按 2！'
        : '暂无';
  ...
  _drawPause(r) {
    ...
    r.text('暂停', GAME.width / 2, GAME.height / 2 - 8, {
      size: 46,
      color: '#ffce54',
      align: 'center',
      weight: '900',
    ...
    r.text('按 P / Esc 继续', GAME.width / 2, GAME.height / 2 + 34, {
      size: 16,
      color: '#cfc6e8',
      align: 'center',
      weight: '700',
  ...
  _drawFailed(r) {
    ...
    r.text(title, GAME.width / 2, GAME.height / 2 - 26, {
      size: 44,
      color: '#ff7a6a',
      align: 'center',
      weight: '900',
    ...
    const reason =
      this.result?.reason === 'broken' ? '原因：木筏耐久归零' : '原因：倾斜超过 ±40° 太久';
    const tip =
      this.result?.reason === 'broken'
        ? '提示：耐久低时，找平稳窗口按 1 修理'
        : '提示：优先用 A/D 稳住木筏';
    r.text(reason, GAME.width / 2, GAME.height / 2 + 10, {
      size: 14,
      color: '#cfc6e8',
      align: 'center',
      weight: '800',
    });
    r.text(tip, GAME.width / 2, GAME.height / 2 + 34, {
      size: 14,
      color: '#fff2b0',
      align: 'center',
      weight: '800',
    });
    r.text('Enter / Space：重新挑战', GAME.width / 2, GAME.height / 2 + 66, {
      size: 16,
      color: '#fff2b0',
      align: 'center',
      weight: '800',
    });
    r.text('Esc：返回流沙河', GAME.width / 2, GAME.height / 2 + 92, {
      size: 14,
      color: '#cfc6e8',
      align: 'center',
      weight: '700',
  ...
  _drawSuccess(r) {
    r.rect(0, 0, GAME.width, GAME.height, 'rgba(5,4,10,0.7)');
    r.text('成功渡过流沙河！', GAME.width / 2, GAME.height / 2 - 26, {
      size: 40,
      color: '#ffce54',
      align: 'center',
      weight: '900',
    ...
    const s = this.state;
    r.text('你完成了：', GAME.width / 2, GAME.height / 2 + 10, { size: 14, color: '#cfc6e8', align: 'center', weight: '900' });
    const lines = [
    ...
    lines.forEach((ln, i) => {
      r.text(ln, GAME.width / 2, GAME.height / 2 + 36 + i * 18, {
        size: 13,
        color: i === 3 ? '#fff2b0' : '#cfc6e8',
        align: 'center',
        weight: '800',
      });
    });
    r.text('按 Space / Enter 返回流沙河', GAME.width / 2, GAME.height / 2 + 126, {
      size: 16,
      color: '#fff2b0',
      align: 'center',
      weight: '800',
  ...
  _drawIntro(r) {
    ...
    r.text(`${remain} 秒后开始 / 按 Space 立即开始`, GAME.width / 2, y + h - 34, {
      size: 16,
      color: '#fff2b0',
      align: 'center',
      weight: '900',
  ...
  _drawSideKeys(r, s) {
    ...
    const leftHot = need === 'A';
    const rightHot = need === 'D';

    this._drawSideKeyCard(r, 22, 160, 132, 118, 'A', '向左稳', leftHot, blink, absA);
    this._drawSideKeyCard(r, GAME.width - 154, 160, 132, 118, 'D', '向右稳', rightHot, blink, absA);
  }
  ...
  _drawSideKeyCard(r, x, y, w, h, key, sub, hot, blink, absA) {
    ...
    r.text(key, x + w / 2, y + 58, {
      size: 56,
      color: hot ? '#fff2b0' : '#cfc6e8',
      align: 'center',
      weight: '900',
      shadow: 'rgba(0,0,0,0.7)',
    });
    r.text(sub, x + w / 2, y + 92, {
      size: 14,
      color: hot ? '#fff2b0' : '#9a90b8',
      align: 'center',
      weight: '900',
    });
    if (hot && absA > 30) {
  ...
  _drawMainPrompt(r, s) {
    ...
    r.text(prompt, GAME.width / 2, 145, {
      size: 16,
      color: '#fff2b0',
      align: 'center',
      weight: '900',
